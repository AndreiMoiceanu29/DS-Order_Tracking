pipeline {
    ...
    options {
        checkoutToSubdirectory('qs_order_tracking')
    }

    stages {
        stage('Clean'){
            steps {
               dir("qs_order_tracking") {
        ...
        stage('create venv') {
            steps {
                sh 'python -m venv qs_order_tracking_env'
            }
        }
                    ...
                    dir("qs_order_tracking") {
                        dir('automations'){
                            checkout([$class: 'GitSCM',
                                    ...
                                    url: 'https://github.com/AndreiMoiceanu29/DSD-Order_Tracking.git']]]
                            )
                            echo "Move files to automations folder"
                            sh 'mkdir roles/repository/files/service'
                            sh 'mv ../order_tracking/* roles/repository/files/service/'
                            withCredentials(
                                [
